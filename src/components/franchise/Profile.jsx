...
const Profile = () => {
  ...
  const handlePanSubmit = async (e) => {
    ...
    try {
      ...
      setPanSuccess(response.data.message || 'PAN number updated successfully!');
    } catch (err) {
  ...
  const handleFetchPanDetails = async () => {
    ...
    if (!panRegex.test(panData.panNumber)) {
      setPanError('Invalid PAN number format. Please enter a valid PAN number (e.g., ABCDE1234F)');
      return;
    }
    ...
    try {
      const response = await franchiseAPI.fetchPanComprehensive({ panNumber: panData.panNumber });
      setPanSuccess(response.data.message || 'PAN details fetched successfully!');
      ...
      setPanError(err.response?.data?.message || 'Failed to fetch PAN details');
      console.error('Error fetching PAN details:', err);
    } finally {
      setPanLoading(false);
    }
  ...
  const handleBankSubmit = async (e) => {
    ...
    try {
      ...
      setBankSuccess(response.data.message || 'Bank details updated successfully!');
    } catch (err) {
  ...
  const handleVerifyBankDetails = async () => {
    ...
    if (!ifscRegex.test(bankData.bankIfscCode)) {
      setBankError('Invalid IFSC code format. Please enter a valid IFSC code (e.g., SBIN0002499)');
      return;
    }
    ...
    try {
      ...
      setBankSuccess(response.data.message || 'Bank details verified successfully!');
      ...
      setBankError(err.response?.data?.message || 'Failed to verify bank details');
                  ...
                  <Grid item xs={12}>
                    <Box sx={{ display: 'flex', gap: 2 }}>
                      <Button
                        ...
                        sx={{ py: 1.5, px: 4 }}
                      >
                        {panLoading ? <CircularProgress size={24} /> : 'Fetch PAN Details'}
                      </Button>
                    </Box>
                  </Grid>
                  ...
                  <Grid item xs={12}>
                    <Box sx={{ display: 'flex', gap: 2 }}>
                      <Button
                        ...
                        sx={{ py: 1.5, px: 4 }}
                      >
                        {bankLoading ? <CircularProgress size={24} /> : 'Verify Bank Details'}
                      </Button>
                    </Box>
                  </Grid>
                      ...
                      <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={6}>
                          <TextField
                            fullWidth
                            label="Bank Name"
                            value={bankDetails.data.ifsc_details?.bank_name || ''}
                            InputProps={{
                              readOnly: true,
                            }}
                          />
                        ...
                        <Grid item xs={12} sm={6}>
                          <TextField
                            fullWidth
                            label="Branch"
                            value={bankDetails.data.ifsc_details?.branch || ''}
                            InputProps={{
                              readOnly: true,
                            }}
                          />
